...
class TestingsController extends Controller
{
	...
	/**
	 * Store a newly created testing in database.
	 *
	...
	{
		$values = DB::table('testings')->select($this->listing_cols)->whereNull('deleted_at');
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) {
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laraadmin.adminRoute') . '/testings/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
